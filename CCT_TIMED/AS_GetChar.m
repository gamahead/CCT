function [ch, when] = GetChar(getExtendedData, getRawCode, timeOut)
...
%
% 8/20/09   as   Added option 'timeOut' to set time limit in secs) for function
%               ALSO assigning 'x' to charAvail if timeOut to avoid errors
%               (temporary?)
...
if psychusejava('desktop')
    ...
    keepChecking = 1;
    while keepChecking && (GetSecs<(to_t1+timeOut))
