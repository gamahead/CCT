...
% How much time to allow per trial?
    p.trialTimeLimit = 45; %EG: Original 45
...
p.baseXshift = 100; %EG: Original 125
p.baseYshift = 275;  %EG: Original 250
p.Xsep = 163; %EG: Original 100
p.Ysep = 160; %EG: Original 125
...
% Using d_width to shift everything on the screen over
d_width = -293;
p.ncRect = [540+d_width 175 665+d_width 225]; %EG: [615 175 740 225]
p.stopRect = [840+d_width 175 1080+d_width 225]; %EG:[675 175 915 225]
p.nextRect = [1255+d_width 175 1440+d_width 225]; %EG: [1000 175 1185 225]
p.ncText = [550+d_width,188];
p.stopText = [855+d_width,188]; %EG: original [390,188]
p.nextText = [1265+d_width,188]; %EG: Original [600,188]
...
[w,rect]=Screen('OpenWindow',0);%[0 0 1920 1080]); 
p.res = rect(2:4);
[p.centerX,p.centerY] = Screen('WindowSize',0); % p.res(1)/2; % x center of main window
p.centerX = p.centerX/2;
...
AS_DrawFormattedText(w,p.instr1,'center','center',p.black,100,1.3); %75, 30, p.black 
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
Screen('Flip',w);
WaitSecs(2.0);
KbWait(-1);
...
AS_DrawFormattedText(w,p.instr2,'center','center',p.black,100,1.3);
...
while (1) % track movement
    ...
    if buttons(1)       % clicked on scale
        ...
        if ~isempty(cardInd)
            ...
            Screen(w, 'TextSize', 24);
            AS_DrawFormattedText(w,p.instr2,100,30,p.black,100,1.3);
            AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3); 
            w=AS_CCT_drawCards(w,pracP,pracT);
            Screen('Flip',w);
            break
...
w=AS_CCT_drawCards(w,pracP,pracT);
Screen(w, 'TextSize', 20);
AS_DrawFormattedText(w,p.instr3,100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
AS_DrawFormattedText(w,'A Gain Card',1225,280,p.black,10,1.3);
AS_DrawFormattedText(w,'A Loss Card',1225,545,p.black,10,1.3);
...
AS_DrawFormattedText(w,p.instr4,100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
...
Screen(w, 'TextSize', 22);
AS_DrawFormattedText(w,CCTi.CCTinstr4{ni},100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
Screen('Flip',w);
WaitSecs(.5);
KbWait(-1);
...
Screen(w, 'TextSize', 22);
AS_DrawFormattedText(w,p.instr5,100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
...
Screen(w, 'TextSize', 22);
AS_DrawFormattedText(w,CCTi.CCTinstr5{ni},100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
Screen('Flip',w);
WaitSecs(.5);
KbWait(-1);
...
Screen(w, 'TextSize', 22);
AS_DrawFormattedText(w,p.instr6,100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
...
Screen(w, 'TextSize', 22);
AS_DrawFormattedText(w,p.instr7,100,100,p.black,65,1.3);
AS_DrawFormattedText(w,'Please press any key to continue...',100,900,p.black,100,1.3);
...
Screen(w, 'TextSize', 22);
AS_DrawFormattedText(w,p.instr8,100,100,p.black,65,1.3);
Screen('Flip',w);
WaitSecs(.5);
KbWait(-1);
